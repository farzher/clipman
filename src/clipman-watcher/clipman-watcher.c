...
static void SpawnClipManWithArgAndExit(const char* arg, BOOL is_copy) {
...
void WINAPI WinMainCRTStartup(void) {
  ...
  while(GetMessageA(&msg, NULL, 0, 0) > 0) {
    if(msg.message == WM_HOTKEY && msg.wParam == HOTKEY_ID) {
      SpawnClipManWithArgAndExit("-spawned_by_watcher", FALSE);
    } else if(msg.message == WM_CLIPBOARDUPDATE) {
      SpawnClipManWithArgAndExit("-spawned_by_watcher -copy", TRUE);
    }
